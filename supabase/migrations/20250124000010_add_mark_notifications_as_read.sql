...
CREATE OR REPLACE FUNCTION public.mark_notifications_as_read(p_user_id uuid, notification_ids uuid[])
RETURNS void AS $$
DECLARE
    update_count integer;
BEGIN
    -- Log which notifications we're updating
    RAISE LOG 'Marking notifications as read for user %, notifications: %', p_user_id, notification_ids;
